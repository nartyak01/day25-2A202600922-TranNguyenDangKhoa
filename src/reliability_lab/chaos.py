...
def _scenario_passed(result: RunMetrics, scenario: ScenarioConfig) -> bool:
    """Define pass/fail criteria per scenario."""
    if scenario.name == "both_providers_fail":
        return result.static_fallbacks == result.total_requests and result.total_requests > 0
    if scenario.name == "primary_timeout_100":
        return result.fallback_successes > 0
